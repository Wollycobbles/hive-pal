...
export const ObservationsCard = ({
  ...
  const numericStats: { label: string; value: number; sub?: string }[] = [
    observations.strength   != null ? { label: t('observations.strength'),  value: observations.strength  } : null,
    observations.queenCells != null ? { label: t('observations.queenCells'), value: observations.queenCells } : null,
  ].filter(Boolean) as { label: string; value: number; sub?: string }[];

  // Boolean flags
  const booleanFlags: { label: string; value: boolean }[] = [
    observations.queenSeen       != null ? { label: t('fields.queenSeen'),           value: observations.queenSeen       } : null,
    observations.swarmCells      != null ? { label: t('observations.swarmCells'),     value: observations.swarmCells      } : null,
    observations.supersedureCells != null ? { label: t('observations.supersedureCells'), value: observations.supersedureCells } : null,
        ...
        <div className="space-y-6">
          ...
          {(observations.additionalObservations?.length ?? 0) > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {observations.additionalObservations!.map((obs, i) => (
          ...
          {(observations.reminderObservations?.length ?? 0) > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {observations.reminderObservations!.map((obs, i) => (
